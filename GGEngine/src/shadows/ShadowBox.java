 ...
 * shadows (basically the orthographic projection area for the shadow render pass, a bounding box around the camera with the light's direction in it).
 * Updated each frame to optimise the area, making it as small as possible (for optimal shadow map resolution)
 * while not being too small to avoid objects not having shadows when they should.
 * Everything inside the cuboidal area represented by this object will be
 * rendered to the shadow map in the shadow render pass. Everything outside the area will not.
 */
public class ShadowBox {
	// Adds a zone of OFFSET size behind the camera where shadows are rendered
	...
	/**
	 * Creates a new shadow box and calculates some initial values for
	 * the camera's view frustum  width and height of the near and far plane.
	 * 
	 * @param lightViewMatrix - the "view matrix" of the light.
     *      Can be used to transform a point from world space into "light" space
     *      (changes a point's coordinates from being in relation to the world's axis to being in terms of the light's local axis).
     ...
     * to make sure that the box covers the smallest area possible while still ensuring that everything inside the camera's view,
	...
	protected void update() {
		...
		toNear.scale(RenderManager.NEAR_Z);
	...
	/**
	 * Calculates the position of the vertex at each corner of the view frustum in light space (8 vertices, so this returns 8 positions).
	 * 
	 * @param rotation - camera's rotation.
	 * @param forwardVector - the direction that the camera (and the frustum) is aiming
	 * @param centerNear - the center of the frustum's near plane.
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
		...
		// World space calculations for the 8 vertices of the view frustrum
		...
		// Add light space calculations to find the minimum & maximum X, Y, Z values - used to determine the dimensions of the cubeoid
	...
	/**
	 * Calculates the width and height of the near and far planes of the camera's view frustum.
     * Does not have to use the actual far plane distance.
     * Bringing the far plane closer increases shadow resolution and vice-versa.
     * Note that by doing this distant objects will not cast shadows
	 */
	private void calculateWidthsAndHeights() {
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(RenderManager.FOV_ANGLE)));
		nearWidth = (float) (RenderManager.NEAR_Z
				* Math.tan(Math.toRadians(RenderManager.FOV_ANGLE)));
